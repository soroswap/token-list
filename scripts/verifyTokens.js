...
async function verifyNewAssets(directoryPath, assetListPath) {
  ...
  const existingAssetsMap = existingAssetList ? existingAssetList.assets.reduce((map, asset) => {
  ...
  for (const file of assetFiles) {
    ...
    if (existingAsset) {
      ...
      const changes = Object.keys(assetData).filter(key => JSON.stringify(assetData[key]) !== JSON.stringify(existingAsset[key]));
      if (changes.length > 0) {
        console.log(`Changes detected for asset ${assetData.contract}:`, changes);
        if (assetData.issuer) {
          try {
            const newAsset = new StellarSdk.Asset(assetData.code, assetData.issuer);
            const newAssetContract = newAsset.contractId(networkPassphrase);
            if (newAssetContract !== assetData.contract) {
              throw new Error(`Contract mismatch for asset ${assetData.code}: expected ${newAssetContract}, got ${assetData.contract}`);
            }
      ...
      if (assetData.issuer) {
        try {
          ...
          if (issuerDomain !== assetData.domain) {
            throw new Error(`Domain mismatch for asset ${assetData.code}: expected ${issuerDomain}, got ${assetData.domain}`);
          }
          const newAsset = new StellarSdk.Asset(assetData.code, assetData.issuer);
          const newAssetContract = newAsset.contractId(networkPassphrase);
          if (newAssetContract !== assetData.contract) {
